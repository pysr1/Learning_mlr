...
mutate(time = start_date - end_date, time = as.numeric(time), start_day = day(start_date), start_month = factor(months(start_date), ordered = F),  start_wday = factor(wday(start_date, label = T), ordered = F), time_since_book = as.numeric(start_date - insert_date)) %>% 
